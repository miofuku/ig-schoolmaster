...
from utils.model_utils import fine_tune_model, load_model
...
# Load model and tokenizer
    model = GPTNeoForCausalLM.from_pretrained('EleutherAI/gpt-neo-125M')    model.to(config.DEVICE)
    model = load_model(model, 'models/gpt_neo_finetuned.pt')
    ...
    knowledge_graph.load_from_rdf('data/course_knowledge.ttl')
    ...
    while True:
        ...
        response, accuracy = dialog_manager.respond(user_input, course_content, course_qa, model, tokenizer)
