...
max_token_value = tokenized_text.max().item()  # the maximum value of the tokenized numbers
...
class TransformerModel(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.ModuleList([nn.TransformerEncoderLayer(d_model, num_heads, d_model * 4, dropout) for _ in range(num_layers)])
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0:
        model.eval()
        with torch.no_grad():
            train_loss = sum(loss_fn(model(get_batch(train_data)[0]).view(-1, max_token_value + 1), get_batch(train_data)[1].view(-1)).item() for _ in range(eval_iters)) / eval_iters
            ...
            print(f'Step: {iter}, Training Loss: {train_loss:.3f}, Validation Loss: {val_loss:.3f}')
